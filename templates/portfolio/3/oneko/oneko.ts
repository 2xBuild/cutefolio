const ONEKO_ID = "oneko-p3";

export function initOneko(nekoFile: string, persistPosition = true): (() => void) | void {
  if (typeof window === "undefined" || typeof document === "undefined") return;

  const mediaQuery = window.matchMedia?.("(prefers-reduced-motion: reduce)");
  const isReducedMotion = mediaQuery?.matches === true;

  if (isReducedMotion) return;

  // Avoid double-initialization in React Strict Mode/dev; only our portfolio-3 oneko
  ...
  function init(): () => void {
    if (persistPosition) {
      const stored = window.localStorage.getItem("oneko-p3");
      if (stored) {
        try {
          const storedNeko = JSON.parse(stored) as {
            nekoPosX: number;
            nekoPosY: number;
            mousePosX: number;
            mousePosY: number;
            frameCount: number;
            idleTime: number;
            idleAnimation: keyof typeof spriteSets | null;
  ...
  function frame() {
    ...
    if (direction && direction in spriteSets) {
